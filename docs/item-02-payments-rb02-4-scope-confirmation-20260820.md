...
| Live-worker interleaving | Worker A must enter the real `process_bulk_payment_batch()` path and acquire the lease in a short admission transaction that commits before the timing seam. A narrow test-only timing seam pauses A before its subsequent child/decision/projection/audit/callback transaction begins. The test expires A’s committed durable lease, runs Worker B through the real task to acquire the next generation and complete, then resumes A. This boundary is required because holding the batch row lock while pausing A would block B’s `select_for_update()` and make a real takeover impossible. |
...
| Permitted implementation adjustment | The task may split lease admission from downstream processing: lock/read eligible batch and acquire/heartbeat the lease in a short committed admission transaction, then run the existing locked processing transaction after a fresh owner assertion. It may expose a narrowly scoped in-process test barrier only between those two boundaries. The hook is inactive in normal execution and cannot add a production persistence path, policy behavior, provider-runtime behavior, or business outcome. Existing owner-token/generation assertions remain the actual fence. |
