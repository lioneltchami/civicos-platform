...
| Durable decision identity | Add one additive decision model, uniquely keyed by `(batch, lease_generation)`. It stores the canonical policy/input fingerprint, lease owner token, generation, mapped decision outcome, policy state, child counts, reason, and relevant non-PII decision metadata. Replaying the same fingerprint returns the same decision; a changed fingerprint at the same key is rejected; a stale owner/generation cannot write. A later valid lease generation may produce a later decision. |
