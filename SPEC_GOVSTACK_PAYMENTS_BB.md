...
**Version:** 1.0.0 (original) — implementation and this document both updated through 2026-07-27
**Date:** 2026-07-19 (original); last substantively updated 2026-07-27
**Status:** **Implemented, internally verified, and genuinely harness-executed.** All 12 post-implementation GAPs (§22), all 4 items of the P0–P3 completion plan (§23), all 3 Round-3 remediation issues plus 2 secondary findings (§24), the SSRF/validation/migration/serializer-defect closure (§25), the P2G tenant-isolation/SSRF/envelope/throttle/settings remediation (§26), and a Round 2 re-audit's 5 further findings — bill-inquiry `requestId` casing, tenant-registry binding, composite tenant-scoped uniqueness constraints, and an already-paid mark-paid audit-trail gap (§27) — are all done and committed. Full `apps/payments/` suite: **1,760 tests passing, 0 failures** (up from 1,750 at the start of §27's round). The live upstream Cucumber harness against `GovStackWorkingGroup/bb-payments`'s own `test/openAPI` suite **WAS genuinely executed this session** (not merely reasoned about from static reads): 74 scenarios, **65 passed / 9 failed**. Of the 9 failures: 5 are a harness-side duplicate-request artifact, independently confirmed via Django's raw request log showing each PATCH logged twice per scenario — not a CivicOS defect; the other 4 traced to a real (non-mocked) async-dispatch defect — `config/__init__.py` was empty, so `@shared_task`-decorated tasks bound to Celery's unconfigured default app instead of this project's configured instance, causing a real `ConnectionRefused` on `.delay()` outside of tests that mock it out — now fixed by importing the configured `Celery` app at package-init time, with a new regression test confirming the correct app is bound. See §26 for the ledger entry on the P2G tenant-isolation/SSRF/envelope round (a fresh 3-agent adversarial audit found Payments was the one BB not yet at the "READY" bar — 1 CRITICAL, 3 HIGH, 2 MEDIUM — all closed that round) and §27 for the subsequent Round 2 re-audit's 2 HIGH/2 MEDIUM/1 LOW findings (all closed). See `MASTER_BB_CERTIFIABILITY_REPORT.md`'s "Payments BB" section for the full audit-to-fix trail across both rounds.
...
25. [First real harness execution + SSRF/validation/migration closure (2026-07-26)](#25-first-real-harness-execution--ssrfvalidationmigration-closure-2026-07-26)
26. [Round 4 Remediation — P2G Tenant Isolation, SSRF Hardening, Response Envelope, Throttle, Settings Flags (2026-07-27)](#26-round-4-remediation--p2g-tenant-isolation-ssrf-hardening-response-envelope-throttle-settings-flags-2026-07-27)

*(Note added 2026-07-27: §25 above was itself added to the document body in the prior round but never linked from this Table of Contents — corrected here as part of breaking this doc's own recurring "fixed in code, never folded into the ledger" pattern; see §26.)*
...
- [x] `GET /govstack/payments/bills/{billId}` returns correct bill data (implemented against `GovStackBill`, not `FeeSchedule` — see §14.1's mapping table, which itself predates the actual model choice; not revised here to stay in scope for this round). This round's fix pass (2026-07-26/27) added the required `fields=inquiry` query param, tenant-scoped lookup via `X-Platform-TenantId`, and the `{responseCode, reason, requestID}` HTTP 202 envelope on top of the pre-existing correct data — confirmed via a personally-run, fully green `apps/payments` suite (1,746/1,746 final count, 4 runs across the round with zero failures) after the fix agent's changes landed, not just a code read — see §26.
- [x] `POST /govstack/payments/billTransferRequests` creates/updates the corresponding bill-payment record — same basis as above: this round added the required `billInquiryRequestId`/`paymentReferenceID` fields, tenant scoping, and the HTTP 202 envelope, confirmed green in the same test run.
- [x] All endpoints return `Content-Type: application/json`
- [x] Min 8 unit tests in `apps/payments/tests/test_govstack_p2g.py` (already well over minimum per §24.2's confirmed count of ~101 tests prior to this round's additions)
...
- [x] All 4 P2G views implemented (`BillInquiryView`, `BillTransferRequestView`, `MarkBillPaidView`, `TransferRequestStatusView`)
- [x] Bill data mapped to `/bills/{billId}` correctly (against `GovStackBill`, the model actually built for Wave 5 — see the note on §14.4's checkboxes above re: the stale `FeeSchedule` reference in this line and in §14.1)
- [x] Bill payment notification creates/updates the corresponding bill-payment record
- [x] No harness submission yet for P2G specifically — still accurate; only G2P + Voucher (9 features) have upstream harness coverage, confirmed again during this round's live harness run (§25.4, §26)
- [x] 8+ unit tests passing (well over minimum — ~101 P2G tests per §24.2, growing further with this round's additions)

*Note on this whole checklist, added this round (2026-07-27): the P2G CRITICAL tenant-isolation bug and the 3 HIGH spec-conformance gaps (response envelope/status, `X-billerId` header, `fields=inquiry` query param) were all open as of the start of this round and are now fixed, with the full `apps/payments` suite (1,746 tests, final count) personally re-run green 4 times over the course of the fix landing — these boxes should still NOT be read as "harness-certifiable" (P2G has zero upstream Cucumber coverage, see above), only as "implemented and spec-conformant per the live OpenAPI YAMLs, test-suite-verified." See §26 for the full status and `MASTER_BB_CERTIFIABILITY_REPORT.md` for the originating audit.*
...
## 26. Round 4 Remediation — P2G Tenant Isolation, SSRF Hardening, Response Envelope, Throttle, Settings Flags (2026-07-27)

**Trigger.** §25 closed with Payments believed done. A fresh, independent 3-agent adversarial audit was then run across all four of this codebase's GovStack Building Blocks (the same discipline already applied to Documents, Appointments, and Consent) and, unlike those three, found Payments **NOT READY** — full detail in `MASTER_BB_CERTIFIABILITY_REPORT.md`'s "Payments BB" section, which is the authoritative source for this round; this section folds that report's findings into this document's own gap ledger, which is this doc's own repeated failure mode per its history (see the Table-of-Contents note above §25) — captured here explicitly so it does not recur a 4th time.

**What the audit found (severity-ranked; full detail in the master report):**
- **CRITICAL** — P2G endpoints (`GovStackP2GService.get_bill`, `get_transfer_request`, `mark_bill_paid`) had no tenant/ownership scoping at all: any caller with a valid `X-PayerFI-Id` could read another tenant's bill, or mark **any** tenant's bill paid with zero audit trail. `X-Platform-TenantId` was validated for presence/length but never actually used to scope a query.
- **HIGH** — Payments' SSRF callback guard (`govstack_tasks.py`) was a stale, weaker copy of the pattern already hardened in `apps/appointments/tasks.py` and `apps/consent/tasks.py` this session: allowed plain `http://`, didn't block the RFC 6598 CGNAT range or the IANA IETF Protocol Assignments block, and didn't treat a 3xx response after `allow_redirects=False` as a failure.
- **HIGH** — P2G response envelope/status code didn't match the live OpenAPI YAMLs: the spec requires HTTP 202 with `{responseCode, reason, requestID}` on `POST /billTransferRequests`, `GET /transferRequests/{id}`, and `POST /bills/{id}/mark-paid` — but `GET /bills/{billId}` is the one exception: its own live spec (`billInquiryRequest.yml`) uses lowercase-d `requestId`, not `requestID`. (A Round 2 re-audit found this doc and the code had both been fixed against the wrong spec file for that one endpoint — see §27 below.) CivicOS returned HTTP 200 with bespoke bodies on all four prior to this round's fix.
- **HIGH** — `GET /transferRequests/{id}` was wired to `IsTrustedPayerFI` (`X-PayerFI-Id`), but the live `rtpStatusUpdateRequest.yml` spec requires `X-billerId` instead, with no PayerFI header anywhere in that file.
- **HIGH** — `GET /bills/{billId}` never read the spec-mandated `required: true` query parameter `fields=inquiry`.
- **MEDIUM** — No BB-identity-keyed throttle: all 13 Payments endpoints shared one generic IP-keyed `ScopedRateThrottle` bucket, unlike the dedicated identity-keyed throttles already added to Appointments and Consent this session.
- **MEDIUM** — `GOVSTACK_REQUIRE_REGISTERED_PAYER_FI`, `GOVSTACK_REQUIRE_PLATFORM_TENANT_ID`, `GOVSTACK_VOUCHER_REQUIRE_JWT`, and `GOVSTACK_VOUCHER_REQUIRE_REGISTERED_BB` were defined only in `production.py` (safe `default=True` there) and absent from `base.py` — harmless only by coincidence, since each flag's own `getattr(..., False)` fallback happened to match the secure default, but the same fail-open trap already fixed for other flags/BBs this session.
- **MEDIUM/needs-confirmation** — the live harness run in §25.4 left 4 of its 9 failures only partially explained ("Timeout reached" smoke failures on bulk-payment/prepayment-validation, not re-litigated at the time). This round's audit traced the real cause: `config/__init__.py` was empty, so every `@shared_task` in this codebase bound to Celery's own unconfigured default app (default broker `amqp://guest@localhost//`, `task_always_eager=False` regardless of `settings.CELERY_TASK_ALWAYS_EAGER`) instead of this project's configured `Celery("civicos")` instance — a real, reproducible defect, not a test artifact, confirmed by direct reproduction of `.delay()` raising `ConnectionRefused` even under `config.settings.test`.
- **LOW/cosmetic** — this document's own masthead was stale (claimed 1,633 tests and "not yet run against the live harness"; both false), `test_govstack_vouchers.py`'s D4/D8 docstrings still said HTTP 400 while their test bodies correctly assert 456, and this §14/§21 Wave 5 checklist was unchecked despite Wave 5 being implemented — all three fixed in this round (see the changes above).

**Fix ownership and status (2026-07-27, this round).** Two sibling fix agents worked in parallel on non-overlapping files, mirroring the established discipline from the Documents/Appointments/Consent rounds:
- **P2G rewrite agent** (`govstack_models.py`, `govstack_services.py`, `govstack_tasks.py`'s celery-app-import fix is actually in `config/__init__.py`, `govstack_auth.py`, the P2G view classes in `govstack_views.py`): added `GovStackBill.platform_tenant_id` (migration `0025_govstackbill_platform_tenant_id.py`), scoped `get_bill()`/`get_transfer_request()`/`mark_bill_paid()` by tenant, rewrote all 4 P2G responses to the `{responseCode, reason, requestID}` / HTTP 202 envelope, added the `fields=inquiry` query-param check on bill inquiry, added all 4 required fields to `BillTransferRequestSerializer`, and added a new `IsTrustedBiller` permission class keyed on `X-billerId` for `TransferRequestStatusView` (replacing the incorrect `IsTrustedPayerFI`/`X-PayerFI-Id` wiring).
- **SSRF/throttle/settings agent** (`govstack_tasks.py`, `govstack_auth.py`'s throttle-identity stashing, base-class throttle wiring in `govstack_views.py`): brought `_is_safe_callback_url()`/`_post_callback()` up to the Appointments/Consent standard (HTTPS-only, CGNAT + IETF-protocol-assignment ranges, 3xx-as-failure check); added BB-identity-keyed throttling (caller identity stashed on `request.META["_gs_payer_identity"]` by `_HeaderWhitelistBBPermission` for the throttle class to key on instead of raw IP); and found and fixed the `config/__init__.py` Celery-app-import root cause of the async-dispatch gap, with a new regression test confirming the correctly-configured app is bound.

**Verified directly by this agent (read-only, independent of both fix agents):** re-read the final `govstack_auth.py`, `govstack_tasks.py`, `govstack_views.py`, `govstack_serializers.py`, and `config/__init__.py` diffs in full; confirmed the tenant-scoping field, the 202 envelope, the `X-billerId`/`IsTrustedBiller` wiring, the `fields=inquiry` check, the SSRF guard's new IP-range constants, and the Celery-app-import fix are all present and internally consistent with the findings above and with `MASTER_BB_CERTIFIABILITY_REPORT.md`'s "Payments BB" section. **Personally re-ran the full `apps/payments` suite four times over roughly 10 minutes as both fix agents' changes continued to land (1,743/1,743 twice, then 1,746/1,746 twice more once 3 additional tests landed): 0 failures at every single run, no regression at any point.** Also personally ran `manage.py check` (clean) and `makemigrations --check --dry-run payments` (`No changes detected in app 'payments'`, clean) after the diff had fully settled. The human orchestrator should still do a final consistency pass — in particular a cross-BB regression run (Documents/Appointments/Consent) to confirm this round hasn't destabilized any of the three already-READY BBs, which this agent did not verify (out of scope for a Payments-only pass) — per the standard closing step for this kind of multi-agent round.

**Independent sweep (this agent, G2P + Voucher only, read-only — no P2G, no models/services/auth/tasks/P2G-views files per this round's ownership split).** Re-checked `RegisterBeneficiaryView`/`UpdateBeneficiaryView`/`BulkPaymentView`/`PrepaymentValidationView`/`PrepaymentValidationResponseView` and all 4 Voucher views, their serializers, and their audit-trail call sites in `govstack_services.py`, against a fresh fetch of the live harness's `helpers.js` schema file and `g2p_register_beneficiary.js` step definitions. **No new findings.** Confirmed: every G2P/Voucher response envelope, status code, and header/query-param behaviour matches the live harness exactly (including the `voucher_status` 7-value enum and the `Cancelled → "Purged"` judgment call, both already correctly implemented); every voucher lifecycle transition (`preactivate`/`activate`/`redeem`/`cancel`) writes a `GovStackPaymentAuditEntry` with no PII in `details`; no bare `{"message": ...}` envelope is used anywhere the harness expects a different shape; ISO 4217 currency validation, `RequestID` length (12 chars), and BB-identifier format validators are all present and correctly routed to their spec-mandated status codes. One pre-existing, already-known-and-owned gap re-confirmed but deliberately not duplicated here: `test_rate_limit.py` has zero dedicated regression coverage for the shared `govstack_bb` `ScopedRateThrottle` scope used by G2P, Voucher, *and* P2G alike (not P2G-specific, contrary to how §24.2 first flagged it) — left for the throttle fix agent above, since fixing it requires touching the same base-class throttle wiring this round's ownership split assigns to them.

---

## 27. Round 2 Remediation — BillInquiry Casing, Tenant-Registry Binding, Composite Uniqueness, Already-Paid Audit Trail (2026-07-27)

**Trigger.** §26 closed with Payments believed fully READY. A fresh, adversarial re-audit (documented in `MASTER_BB_CERTIFIABILITY_REPORT.md`'s "Round 2" / "Payments BB" section) distrusted that "READY" label, re-read the current code from scratch, and re-fetched the live upstream spec files directly rather than relying on §26's own paraphrase of them — the same discipline applied to Documents/Appointments/Consent in that same round. It found 2 new HIGH, 2 new MEDIUM, and 1 new LOW finding, all fixed in this round.

**What this round found and fixed:**

- **HIGH — `BillInquiryView`'s response envelope used the wrong field-name casing, because §26's fix cited the wrong spec file.** §26 (and this document, and the test suite) all matched `BillInquiryView`'s `requestID` casing against `billInquiryResponse.yml` — but that file describes a DIFFERENT, reverse-direction endpoint (`POST /bills/{billId}`, the Payments BB calling OUT to the Payer FI as an async callback), not the `GET /bills/{billId}` PayerFI→PBB endpoint `BillInquiryView` actually implements. A fresh fetch of the correct file, `billInquiryRequest.yml`, confirmed its response schema uses lowercase-d `requestId` at both HTTP 202 and 400. The other 3 P2G endpoints (`billPaymentRequest.yml`, `rtpStatusUpdateRequest.yml`, `markBillPpaymentRequest.yml`) correctly use `requestID` and were left unchanged.
  ✅ FIXED. `GovStackAPIView._validate_platform_tenant_id()` gained an optional `request_id_key: str = "requestID"` parameter (used in both of its own 400 response bodies instead of a hardcoded literal); `BillInquiryView.get()` now calls it with `request_id_key="requestId"`. The view's own two hardcoded `"requestID"` response bodies (the `fields=inquiry` 400 and the success 202) were changed to `"requestId"`. `BillInquiryView`'s docstring, `BillInquiryResponseSerializer` (`govstack_serializers.py`), this document (§26's summary bullet above), and the bill-inquiry-specific assertions in `test_govstack_p2g.py` (section A2) were all corrected. The other 3 views' call sites and assertions are untouched.

- **HIGH — tenant scoping was a self-asserted claim, not a verified identity binding.** §26's tenant-scoping fix scoped every P2G query by whatever `platform_tenant_id` string the caller declared in `X-Platform-TenantId`, but nothing verified the calling, whitelisted PayerFI/biller was actually entitled to declare that specific tenant ID — a caller who simply knew or guessed another tenant's ID string got full cross-tenant read/write access, the same class of bug as before, just requiring one extra guessable string.
  ✅ FIXED, as a minimal, backward-compatible, OPT-IN registry — not the full multi-tenant data-isolation redesign `_validate_platform_tenant_id()`'s own docstring previously argued against (that reasoning is now updated to explain why the calculus changed: this finding is real and exploitable, not speculative). `GovStackRegisteredBB` (`govstack_models.py`) gained `allowed_platform_tenant_ids = models.JSONField(default=list, blank=True, ...)` — migration `0026_govstackregisteredbb_allowed_platform_tenant_ids_and_more.py`. `_validate_platform_tenant_id()` now performs an additional check, gated on `GOVSTACK_REQUIRE_PLATFORM_TENANT_ID=True` AND a resolved caller identity (`request.META["_gs_payer_identity"]`, stashed by `_HeaderWhitelistBBPermission` when a whitelisted BB is granted access): if that caller's `GovStackRegisteredBB` row has a non-empty `allowed_platform_tenant_ids`, the declared tenant id must be in that list, else HTTP 400 (reusing the `request_id_key` parameter from the fix above for envelope consistency). An EMPTY list (the default) is unrestricted — deliberately opt-in, since there is no existing tenant-registry data to migrate and a hard-deny default would break every current caller with zero migration path. `GovStackRegisteredBBAdmin` (`admin.py`) gained a "P2G Tenant Scoping" fieldset exposing the new field. New `TestPlatformTenantIdRegistryBinding` test class (`test_govstack_p2g.py`, section D3, 6 tests) pins: a restricted caller declaring a disallowed tenant → 400; the same caller declaring its allowed tenant → 202; an unrestricted (empty-list) caller is unaffected; a caller with no resolved identity (harness mode) skips the check entirely; an inactive registered BB is rejected earlier, at the permission layer.

- **MEDIUM — tenant scoping was added to reads/writes but not to uniqueness constraints.** `GovStackBill.bill_id` and `GovStackBillPayment.request_id` remained globally `unique=True`, not scoped to `platform_tenant_id` — a coincidental ID collision between two unrelated tenants sharing one deployment would incorrectly reject the second tenant's legitimate request as a duplicate of the first's.
  ✅ FIXED. `unique=True` removed from both fields (replaced with `db_index=True` to preserve lookup performance); `models.UniqueConstraint(fields=["platform_tenant_id", "bill_id"], name="gs_bill_tenant_billid_uniq")` and `models.UniqueConstraint(fields=["platform_tenant_id", "request_id"], name="gs_billpayment_tenant_requestid_uniq")` added to each model's `Meta.constraints`, alongside the existing `CheckConstraint`s. Same migration as above generated both `AlterField` operations (dropping the old unique index) and both `AddConstraint` operations correctly in one file — verified via `makemigrations --check --dry-run payments` reporting clean afterward. This preserves today's behaviour for blank-tenant (`platform_tenant_id=""`) records — two blank-tenant bills/payments still collide, exactly as before (pinned by the existing F1/G1 tests, which use the default blank tenant) — while allowing two different tenants to legitimately reuse the same bill_id/request_id string (new F9/F10, G5/G6 tests). `govstack_services.py`'s `IntegrityError` handling in `create_transfer_request()` catches broadly, without inspecting the constraint name, so no behavioural change was needed there beyond an updated code comment noting the new constraint name.

- **LOW — no audit trail for a mark-paid call against an already-PAID bill.** `mark_bill_paid()` only wrote a `GovStackPaymentAuditEntry` inside the `if bill.status != STATUS_PAID:` branch — a legitimate, authenticated call against an already-paid bill left zero evidence it was ever attempted.
  ✅ FIXED. An audit entry is now written in the already-paid branch too, reusing `ACTION_BILL_PAID` (no new action choice needed) and disambiguated via `details={"trigger": "already_paid_noop", "bill_pk": ...}` — the same `details.trigger` convention already used by `apps.consent.services` to distinguish its `"signature_attached"` vs. `"signature_updated"` entries under one action choice. The real UNPAID→PAID transition entry's `details` dict also gained `"trigger": "status_transition"` for symmetry. Existing tests `C10` (view-level) and `E18` (service-level) — which previously asserted NO new entry for the already-paid case — were updated to assert the new no-op entry IS created, with the correct `trigger`/`actor_bb_id`; a new `E18b` test separately pins that the bill's status/response remain unchanged in this case (the behaviour the old E18 was really guarding).

**Fix ownership.** All 4 findings above were fixed as a single-owner pass scoped strictly to `apps/payments/` (this repo's other 3 GovStack BBs — Documents, Appointments, Consent — plus `config/` were being fixed concurrently by 2 sibling agents in the same working tree and are explicitly out of scope here).

**Files changed:** `apps/payments/govstack_views.py` (`_validate_platform_tenant_id()` parameterized + tenant-registry check; `BillInquiryView` casing + docstring), `apps/payments/govstack_models.py` (`GovStackRegisteredBB.allowed_platform_tenant_ids`; `GovStackBill`/`GovStackBillPayment` composite `UniqueConstraint`s), `apps/payments/govstack_services.py` (`mark_bill_paid()` already-paid audit entry; `IntegrityError` comment update), `apps/payments/govstack_serializers.py` (`BillInquiryResponseSerializer`), `apps/payments/admin.py` (`GovStackRegisteredBBAdmin` fieldset), `apps/payments/migrations/0026_govstackregisteredbb_allowed_platform_tenant_ids_and_more.py` (new), `apps/payments/tests/test_govstack_p2g.py` (assertion fixes + new `TestPlatformTenantIdRegistryBinding`, F9/F10, G5/G6, E18b test classes/methods), this document.

**Verification.** Full `apps/payments` suite: **1,760/1,760 pass, 0 failures** (1,750 baseline + 10 new tests this round). `manage.py check payments`: clean. `makemigrations --check --dry-run payments`: `No changes detected in app 'payments'`, clean.
