...
def validate(path: pathlib.Path) -> list[str]:
    ...
    if data.get("scope") != "repository-only-deterministic":
        errors.append("scope must be repository-only-deterministic")
    if not isinstance(data.get("tests"), list) or not data["tests"]:
        errors.append("tests must be non-empty list")
    if not data.get("external_deferred"):
        errors.append("external_deferred must be non-empty")
    return errors
