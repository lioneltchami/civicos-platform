...
REQUIRED = {
    "schema_version", "rehearsal_id", "authorization", "target", "release",
    "ownership", "controls", "outcome"
}
...
def validate(path: Path) -> list[str]:
    ...
    if target.get("network_action_permitted") is not False or target.get("synthetic_data_only") is not True:
        errors.append("manifest must prohibit network actions and require synthetic data")
    for value in (data.get("rehearsal_id"), auth.get("approval_reference"), target.get("alias"), release.get("commit_sha"), release.get("image_digest"), release.get("migration_set"), release.get("configuration_fingerprint"), ownership.get("deployment_owner"), ownership.get("rollback_owner"), ownership.get("evidence_custodian"), controls.get("rollback_plan_reference"), controls.get("evidence_location")):
        if not isinstance(value, str) or not value or "UNSET" in value or "REQUIRED" in value:
            errors.append("unresolved required field")
            break
    if not isinstance(controls.get("abort_criteria"), list) or not controls["abort_criteria"] or "UNSET" in controls["abort_criteria"]:
...
if __name__ == "__main__":
    manifest = Path(sys.argv[1]) if len(sys.argv) == 2 else Path("docs/staging/rehearsal-manifest.example.json")
