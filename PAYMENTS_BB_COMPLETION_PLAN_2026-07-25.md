...
## STATUS UPDATE (2026-07-25, same day): P1 — DONE

Implemented via 3 parallel agents (1 implementer, 1 independent live-harness re-verifier, 1 codebase-side pre-flight auditor), then personally re-verified before commit (read every diff hunk, re-read the full `redeem()` call flow, re-read the full test file's 462/463 coverage, ran `manage.py check`/`test apps.payments`/`makemigrations --check --dry-run` myself). Commit `cf49bda`.

- All 4 voucher-engine response bodies rewritten to the real harness schema: preactivation → `{voucher_number, voucher_serial_number, expiry_date_time}`; activation/redemption → `{result_status}`; status-check GET → `{voucher_status (7-value enum string), voucher_amount (string)}`; cancellation PATCH → `{message}`.
- `_VOUCHER_STATUS_ENUM_MAP` added, mapping every real model state (`PREACTIVATED`, `ACTIVATED`, `SUSPENDED`, `BLOCKED`, `PURGED`, and `CANCELLED` as a judgment call → `"Purged"`) to the exact enum casing the harness expects. `"Not Existing"` is left genuinely unreachable (456 already covers "not found"), documented as such — not a gap, a redundant enum value in the upstream schema.
- `get_status()` now checks `STATUS_CONSUMED` → 458 before checking expiry → 459, replacing the previous magic-string/no-op logic.
- `Gov_Stack_BB` blocklist (`_is_known_invalid_gov_stack_bb()`) and numeric `voucher_number` validation (`_is_numeric_voucher_number()`) wired into preactivate/activate/redeem, matching the harness's actual blocklist-not-allowlist fixture behavior documented in the original plan (line 50 below).
- Cancellation (`VoucherStatusCheckView.patch()`) reworked exactly per the plan's §P1.6: both `voucherserialnumber` and `Gov_Stack_BB` now required in the body (400 if missing), invalid `Gov_Stack_BB` → 463 (not 460, confirmed correct per the harness's own unusual reuse of 463 on this one endpoint), response now includes `"message"`.
- **The plan's own §51/§97 caveat — that 462 vs 463 "cannot be fully reverse-engineered from client-side Gherkin fixtures alone" and would need to ship as an admittedly-best-effort heuristic — turned out to be resolvable, not a permanent limitation.** The independent live-harness re-verification agent checked a location neither the original plan-writing pass nor the P1 implementer had checked: `examples/mock-bb-payments/mockoon-paymentsbbvoucher.json` in `GovStackWorkingGroup/bb-payments` — the actual reference/certification mock server config. It defines the disambiguating rule explicitly, on the exact `(merchant_name, merchant_bank_details)` pair: `("Ronan Oliver", "Vigor Bank Group")` → 462 (`InsufficientFunds`), `("Annie Krueger", "Omega Holding Company")` → 463 (`CannotCreditMerchant`). This is corroborated independently by `voucher_redemption.js`'s hardcoded When-steps and `test-data.json`'s merchant fixture comments — 3 independent sources agree. `_classify_redemption_decline()` now implements this rule directly (falling back to the group-sentinel heuristic only for any other merchant pair, which the harness never actually exercises). Both 462 and 463 are now definitively resolved, not best-effort.
- Personally added tests proving the exact fixture pairs are reachable (not just the group-sentinel fallback the implementer had originally tested): `test_c15b`/`test_c15c` at the API layer, `test_f21f`/`test_f21g` at the service layer.
- Personally corrected stale "best-effort"/"NOT currently reachable" language left over from the original plan in `VoucherRedemptionView`'s class docstring, the `redeem()` inline comment, and the exception class docstrings for `InsufficientFunds`/`CannotCreditMerchant` — these now cite the Mockoon discovery instead of describing 463 as an unreachable schema-completeness placeholder.
- `python manage.py check`: 0 issues. `python manage.py test apps.payments`: 1615/1615 passing (up from 1590 after P0, +25 net from the schema rewrite + new tests). `python manage.py makemigrations --check --dry-run payments`: only the pre-existing, already-documented P3 cosmetic drift — no new drift from P1.
- 455 (`VoucherGroupExhausted`) remains implemented-but-untested, as the plan always flagged — no Gherkin scenario exists for it anywhere in the upstream repo, confirmed again this round.

P2 (`Gov_Stack_BB` production allowlist via `GovStackRegisteredBB` + seed fix) and P3 (mechanical migration cleanup) remain open and unstarted — see their sections below, unchanged from the original plan.
