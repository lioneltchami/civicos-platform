...
class IdempotencyService:
    ...
    @staticmethod
    def reserve(*, tenant_id: str, method: str, path: str, key: str, payload: Any):  # noqa: ANN205
        ...
        with transaction.atomic():
            try:
                ...
                if row.fingerprint != fingerprint:
                    raise ValueError("idempotency key was reused with a different payload")  # noqa: B904
                return row, False

    @staticmethod
    def complete(row, *, status_code: int, body: dict, headers: dict | None = None) -> None:  # noqa: ANN001
        ...
        row.save(
            update_fields=["status_code", "body", "headers", "state", "completed_at", "updated_at"]
        )

    @staticmethod
    def replay(row):  # noqa: ANN001, ANN205
...
class ReconciliationReportView(APIView):
    ...
    def get(self, request):  # noqa: ANN001, ANN201
        ...
        qs = (
            PaymentReconciliation.objects.filter(attempt__tenant_id=scope.tenant_id)
