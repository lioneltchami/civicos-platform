...
def test_deterministic_provider_timeout_then_status_settled():
    p = DeterministicProvider(
        [ProviderResult(ProviderOutcome.TIMEOUT, retryable=True)],
        {"tx-1": ProviderResult(ProviderOutcome.SETTLED, external_transaction_id="tx-1")},
    )
...
def test_idempotency_replay_conflict_and_route_method_tenant_scope():
    a = canonical_fingerprint(
        tenant="t1", method="post", path="/payments/", key="k", payload={"x": 1}
    )
    b = canonical_fingerprint(
        tenant="t1", method="POST", path="payments", key="k", payload={"x": 1}
    )
    c = canonical_fingerprint(
        tenant="t2", method="POST", path="payments", key="k", payload={"x": 1}
    )
    assert a == b and a != c
...
def test_status_and_batch_exclude_settled_items():
    ...
    result = evaluate(
        [{"id": "1", "status": "settled"}, {"id": "2", "status": "retryable"}],
...
def test_batch_policy_settled_and_rejected_is_terminal_partial():
    result = evaluate(
        [{"id": "1", "status": "settled"}, {"id": "2", "status": "rejected"}], failure_threshold=1.0
    )
...
def test_batch_policy_threshold_pause_is_explicit():
    result = evaluate(
        [{"id": "1", "status": "settled"}, {"id": "2", "status": "uncertain"}],
