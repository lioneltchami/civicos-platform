...
class PrepaymentValidationNotCompleted(PrepaymentExecutionGateError):
    pass


class PrepaymentValidationFailed(PrepaymentExecutionGateError):
    pass


class PrepaymentBeneficiaryNotFound(PrepaymentExecutionGateError):
    pass


class PrepaymentFinancialAddressInvalid(PrepaymentExecutionGateError):
    pass


class PrepaymentExecutionKeyConflict(PrepaymentExecutionGateError):
    pass


def admit_prepayment_execution(*, validation_request_id, execution_key: str) -> PrepaymentExecution:
    ...
    with transaction.atomic():
        request = PrepaymentValidationRequest.objects.select_for_update().get(pk=validation_request_id)
        ...
        if existing is not None:
            if existing.execution_key != key:
                raise PrepaymentExecutionKeyConflict("validation request already has another execution key")
            return existing
        try:
            with transaction.atomic():
            ...
            if existing.validation_request_id != request.pk:
                raise PrepaymentExecutionKeyConflict("execution key belongs to another validation request")
            return existing
