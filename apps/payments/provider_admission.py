...
def trusted_tenant_id(
    *, authenticated_tenant: str | None, declared_tenant: str | None = None
) -> str:
...
def admit_provider_attempt(
    *,
    ...
    tenant_id = trusted_tenant_id(
        authenticated_tenant=trusted_tenant or tenant_id, declared_tenant=tenant_id
    )
    operation, request_key, currency = (
        (operation or "").strip(),
    ...
    with transaction.atomic():
        ...
        if (
            not registration.provider_name
            or not registration.configuration_version
            or not isinstance(registration.configuration, dict)
        ):
            raise AdmissionError("provider configuration is malformed")
        ledger, created_ledger = IdempotencyLedger.objects.get_or_create(
            tenant_id=tenant_id,
            method="POST",
            path=f"provider:{operation}",
            key=request_key,
        ...
        if not created_ledger:
            ...
            return AdmissionResult(
                existing, admitted=True, replayed=True, idempotency_replayed=True
            )
        ...
        transaction.on_commit(
            lambda attempt_id=str(attempt.pk): enqueue_attempt(
                PaymentAttempt.objects.get(pk=attempt_id)
            )
        )
