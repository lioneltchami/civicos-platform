...
def generate_and_send_receipt(self, receipt_pk: str) -> dict:  # noqa: ANN001
    """
    ...
    try:
        receipt = OfficialDonationReceipt.objects.select_related("donation", "donation__donor").get(
            pk=receipt_pk
        )
    except OfficialDonationReceipt.DoesNotExist:
        logger.error("payments.task.receipt_not_found receipt_pk=%s", receipt_pk)
    ...
    try:
        with transaction.atomic():
            receipt_locked = (
                OfficialDonationReceipt.objects.select_related(
                    "donation", "donation__donor", "document"
                )
            ...
            if receipt_locked.document_id:
                ...
                was_generated = True
                # document FK is persisted by save_receipt_pdf() via _base_manager.update()
    ...
    # Re-fetch after the lock is released to get the document FK populated by
    # save_receipt_pdf() (which writes it via _base_manager.update(), leaving the
    # in-memory receipt_locked object stale). A fresh .get() with select_related
    # fetches both the FK column and the related Document object in one query.
    receipt_locked = OfficialDonationReceipt.objects.select_related("document").get(pk=receipt.pk)

    if pdf_bytes is None:
        # PDF was already on storage (generated by another worker or a previous run)
        from django.core.files.storage import default_storage

        try:
            with default_storage.open(receipt_locked.document._storage_key, "rb") as fh:
                pdf_bytes = fh.read()
        except Exception as exc:
            logger.error(
...
def generate_annual_receipts(self, tax_year: int) -> dict:  # noqa: ANN001
    """
    ...
    try:
        ...
        logger.error("payments.task.annual_receipts.no_charity tax_year=%s", tax_year)
    ...
    _CANADA_NL_TZ = ZoneInfo("America/St_Johns")  # noqa: N806
    _CANADA_WESTERN_TZ = ZoneInfo("America/Vancouver")  # noqa: N806

    _year_start_utc = datetime(tax_year, 1, 1, 0, 0, 0, tzinfo=_CANADA_NL_TZ).astimezone(UTC)
    _year_end_utc = datetime(
        tax_year, 12, 31, 23, 59, 59, 999999, tzinfo=_CANADA_WESTERN_TZ
    ).astimezone(UTC)
    ...
    with transaction.atomic():
        for donor_id, donor_group in itertools.groupby(
            ...
            try:
                ...
                eligible_donations = [
                    d for d in donor_donations if d.eligible_amount > Decimal("0.00")
                ]
                ...
                unique_advantage_descriptions = "; ".join(
                    desc
                    for desc in dict.fromkeys(  # preserve order, deduplicate
                        d.advantage_description
                        for d in eligible_donations
                        if getattr(d, "advantage_description", None)
                    )
                )
                advantage_description = (
                    unique_advantage_descriptions[:255] if unique_advantage_descriptions else ""
                )
                ...
                try:
                    with transaction.atomic():
                        receipt = OfficialDonationReceipt(
                            ...
                            donor_address_line1=donor_address_parts.get(
                                "line1", donor_address_snapshot[:255]
                            ),
                        ...
                        transaction.on_commit(
                            lambda r=receipt: generate_and_send_receipt.delay(str(r.pk))
                        )
                    ...
                    _UNIQUE_CONSTRAINT = "payments_receipt_unique_issued_per_donation"  # noqa: N806
                ...
                logger.error(
                    "payments.task.annual_receipts.donor_error " "tax_year=%s error_type=%s",
                    tax_year,
                    type(exc).__name__,
                )
                failed += 1
    ...
    logger.info(
        "payments.task.annual_receipts.done " "tax_year=%s processed=%s skipped=%s failed=%s",
        tax_year,
        processed,
        skipped,
        failed,
...
def kickoff_annual_receipts(self) -> dict:  # noqa: ANN001
    """
