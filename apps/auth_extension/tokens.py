...
def get_or_create_guest_token(request) -> str:  # noqa: ANN001
...
class GuestTokenManager:
    @staticmethod
    def get(request) -> str | None:  # noqa: ANN001
        return request.session.get(GUEST_SESSION_KEY)

    @staticmethod
    def create(request) -> str:  # noqa: ANN001
        token = generate_guest_token()
        request.session[GUEST_SESSION_KEY] = token
        request.session.modified = True
        return token
    ...
    @staticmethod
    def clear(request) -> None:  # noqa: ANN001
