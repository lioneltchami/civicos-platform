...
def flush_expired_jwt_tokens(self):  # noqa: ANN001, ANN201
    """
    ...
    try:
        ...
        logger.info("flush_expired_jwt_tokens: deleted %d expired token(s)", deleted_count)
        return {"deleted": deleted_count}
    except Exception as exc:
        logger.exception("flush_expired_jwt_tokens failed: %s", exc)
        raise self.retry(exc=exc, countdown=60 * 60)  # retry in 1 hour  # noqa: B904
