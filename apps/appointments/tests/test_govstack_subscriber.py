...
class SubscriberNewTests(SubscriberBaseTestCase):
    ...
    def test_s2_post_new_missing_email_returns_400(self):
        ...
        qry = {"subscriber_details": {"name": "No Email", "category": "individual"}}
        resp = self._post(qry)
    ...
    def test_s3_post_new_duplicate_email_returns_400(self):
        ...
        qry = {"subscriber_details": {"name": "Second User", "email": email}}
        resp = self._post(qry)
        self.assertEqual(resp.status_code, 400)
        data = resp.json()
        self.assertEqual(data["status"], "error")
    ...
    def test_s6_post_new_without_auth_returns_4xx(self):
        ...
        """
        qry = json.dumps({"subscriber_details": {"email": "noauth@example.com"}})
    ...
    def test_s31_post_new_invalid_email_format_returns_400(self):
        ...
        resp = self._post({"subscriber_details": {"name": "Alice", "email": "not-an-email"}})
        self.assertEqual(resp.status_code, 400)
        data = resp.json()
        self.assertEqual(data["status"], "error")
    ...
    def test_s34_post_new_error_message_does_not_contain_email(self):
        ...
        resp = self._post({"subscriber_details": {"email": "existing@example.com"}})
        self.assertEqual(resp.status_code, 400)
    ...
    def test_s38_post_new_name_too_long_returns_400(self):
        ...
        long_name = "A" * 151 + " B"
        qry = {"subscriber_details": {"email": "longname@example.com", "name": long_name}}
        resp = self._post(qry)
        self.assertEqual(resp.status_code, 400)
    ...
    def test_s39_post_new_category_too_long_returns_400(self):
        ...
        qry = {"subscriber_details": {"email": "longcat@example.com", "category": "X" * 51}}
        resp = self._post(qry)
        self.assertEqual(resp.status_code, 400)
...
class SubscriberSpecWireFormatTests(SubscriberBaseTestCase):
    ...
    OpenAPI spec's subscriber_new_qry schema — i.e. the `qry` query
    PARAMETER's JSON value is {"subscriber_details": {...}}, single-nested,
    with no additional wrapper key — must succeed.
    ...
    spec's contract. (A second, unrelated bug — this codebase additionally
    double-wrapping the whole thing under an extra outer "qry" key — was
    fixed separately; the shape asserted here has always been single-nested
    once FIX 1 renamed the inner key.) This test intentionally does NOT
    reuse any shared payload-building helper: it hardcodes the wire-format
    dict inline so a future accidental revert of the inner details key fails
    loudly here, independent of any other test in this module (matching the
    identical precedent established by MessageSpecWireFormatTests /
