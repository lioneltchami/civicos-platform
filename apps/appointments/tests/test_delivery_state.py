...
def test_duplicate_is_idempotent_and_timeout_retries_then_delivers():
    ...
    duplicate = store.create(correlation_id="corr-1", idempotency_key="idem-1", recipient_ref="fake:1")
...
def test_4xx_and_5xx_are_bounded_and_dead_lettered_then_replayed():
    store = DeliveryStore()
    row = store.create(correlation_id="corr-2", idempotency_key="idem-2", recipient_ref="fake:2", max_attempts=2)
...
def test_partial_recipient_failure_and_cancellation_race():
    ...
    bad = store.create(correlation_id="corr-3", idempotency_key="idem-bad", recipient_ref="fake:bad")
...
def test_metrics_are_non_pii_and_authorized_status_is_owner_scoped():
    store = DeliveryStore()
    store.create(correlation_id="corr-5", idempotency_key="idem-5", recipient_ref="secret@example.test")
