...
class GovStackBBCredential(TimestampedModel):
    """
    GovStack Scheduler BB — high-entropy authentication credential.

    Security background (Finding #1 remediation):
      ``GovStackRegisteredBB.bb_id`` (apps.payments.govstack_models) is a
      PUBLIC, non-secret infrastructure identifier by design — its own
      docstring states "No PII is stored here — bb_id is an infrastructure
      identifier, not a citizen ID", it is shown in Django admin
      list_display/search_fields, logged in plaintext warning messages, and
      transmitted as an ordinary REQUIRED request-body field (SourceBBID) on
      Payments G2P endpoints. Prior to this model, GovStackSchedulerAuth
      authenticated callers by checking ``bb_id=request_token`` — i.e. the
      "secret" token WAS the same public identifier. Anyone who learned a
      registered BB's bb_id (trivial, since it is not treated as secret
      anywhere else) could fully authenticate as that BB with its complete
      role privileges. This is a real auth-bypass-by-design, not merely a
      harness-conformance gap.

    Fix: a SEPARATE, high-entropy secret (``secrets.token_urlsafe(32)``),
    hashed with Django's password hasher (never stored or logged in
    plaintext), associated 1:1 with a GovStackRegisteredBB row.
    ``GovStackSchedulerAuth`` now resolves the query param ``requestor_id``
    against ``GovStackRegisteredBB.bb_id`` (identity lookup — fine to stay
    non-secret) and verifies the query param ``request_token`` against THIS
    model's hashed secret via ``check_token()``. The two query param NAMES
    are unchanged (the spec-facing contract is preserved) but their
    semantics are no longer the same value.

    Citizen self-booking decision: GovStackCitizenAuth continues to require
    this same BB-credential gate even for citizen JWT-authenticated calls.
    The calling system (e.g. CivicOS's own citizen portal) holds the secret
    on the citizen's behalf — the citizen is never burdened with managing
    a credential — while still verifying WHICH system is calling, not just
    who the citizen is. This is a deliberate defense-in-depth choice, not
    an oversight; see the module docstring in govstack_auth.py.

    The plaintext secret is generated by the
    ``govstack_generate_bb_credential`` management command, printed to the
    operator exactly once, and never persisted or logged anywhere.
    """

    bb = models.OneToOneField(
        "payments.GovStackRegisteredBB",
        on_delete=models.CASCADE,
        related_name="scheduler_credential",
        verbose_name=_("Registered BB"),
        help_text=_("The GovStackRegisteredBB (public bb_id) this credential authenticates."),
    )
    token_hash = models.CharField(
        max_length=255,
        verbose_name=_("Hashed request token"),
        help_text=_(
            "Hashed via django.contrib.auth.hashers.make_password. "
    ...
    token_prefix = models.CharField(
        max_length=8,
        blank=True,
        verbose_name=_("Token prefix (display only)"),
        help_text=_(
            "First 8 characters of the plaintext token, retained only to help "
            "operators tell rotated credentials apart in the admin. Not a secret "
            "on its own (32+ bytes of entropy remain unknown)."
        ),
    )
    last_used_at = models.DateTimeField(
        ...
        help_text=_("Updated on each successful authentication (best-effort, not transactional)."),
    )
    rotated_at = models.DateTimeField(
        auto_now=True,
        verbose_name=_("Last rotated at"),
    )

    class Meta:
        verbose_name = _("GovStack BB Scheduler Credential")
