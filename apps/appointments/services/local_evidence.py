...
def _status(records: Iterable[Any], owner_id: str, tenant_id: str) -> SchedulerStatus:
    selected = [r for r in records if getattr(r, "owner_key", getattr(r, "owner_id", None)) == owner_id and getattr(r, "tenant_id", tenant_id) == tenant_id]
    counts = {s: sum(getattr(r, "status", getattr(r, "state", None)) == s for r in selected) for s in ("pending", "in_flight", "retry", "dead_letter", "acknowledged", "delivered", "cancelled")}
    return SchedulerStatus(owner_id, tenant_id, counts["pending"], counts["in_flight"], counts["retry"], counts["dead_letter"], counts["acknowledged"], counts["delivered"], counts["cancelled"], len(selected))
...
class LocalSchedulerStatusService:
    ...
    def from_database(self, *, owner_id: str, tenant_id: str, limit: int = 100) -> tuple[SchedulerStatus, list[dict[str, Any]]]:
        ...
        rows = list(qs)
        return _status(rows, owner_id, tenant_id), [{"id": r.pk, "status": r.status, "attempts": r.attempts} for r in rows]


def redact_trace(event: Mapping[str, Any]) -> dict[str, Any]:
    sensitive = {"token", "authorization", "password", "email", "phone", "name", "address", "url", "payload", "recipient", "contact", "message", "credential"}
    return {k: ("[REDACTED]" if k.lower() in sensitive or any(x in k.lower() for x in ("token", "secret", "password")) else v) for k, v in event.items()}
...
def build_local_trace(events: Iterable[Mapping[str, Any]], *, source_revision: str) -> dict[str, Any]:
    by_id = {str(e.get("operation_id")): redact_trace(e) for e in events}
    return {"source_revision": source_revision, "operations": [{"operation_id": i, **by_id[i]} for i in OPERATION_IDS if i in by_id]}
...
def validate_local_evidence(bundle: Mapping[str, Any]) -> tuple[bool, str]:
    ...
    if ids != list(OPERATION_IDS): return False, "exact 37 operation IDs required in deterministic order"
    if bundle.get("source_revision") in (None, ""): return False, "source revision required"
    raw = json.dumps(bundle, sort_keys=True, separators=(",", ":"))
    if bundle.get("checksum") and bundle["checksum"] != sha256(raw.encode()).hexdigest(): return False, "checksum mismatch"
    return True, "valid"


__all__ = ["FakeOutcome", "FakeResult", "LocalFakeAdapter", "LocalPaymentsFake", "LocalConsentFake", "PaymentsFakeAdapter", "ConsentFakeAdapter", "SchedulerStatus", "operational_status", "redact_trace", "LocalAuthorityContract", "LocalSchedulerStatusService", "OPERATION_IDS", "build_local_trace", "validate_local_evidence"]
