...
==================================
HTTP protocol-contract tests for the Document Management Building Block.

This file tests the HTTP-level contract — status codes, Content-Type,
Content-Disposition, redirect targets, and response headers — for every
view in the documents app.  There is no DRF REST API; "API" here means
the Django view HTTP interface consumed by browsers and citizens.
